...
/**
 * create_dnode - creates new node at the list.
 * @n: data of node element
 * @prev: link to previous node
 * @next: link to next following node.
...
{
	dlistint_t *curr = *h;
	dlistint_t *localPrev = NULL;
	unsigned int count = 0;

	if (!h)
		return (NULL);
	if (idx == 0)     /* insertion at the beginning of list*/
	{
		if (!*h)
	...
	if ((count == idx) && (curr == NULL)) /*insertion at list end*/
	{
